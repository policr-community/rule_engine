...
describe RuleEngine do
  describe RuleEngine::Rule do
    it "self_contained?" do
      ...
      content =
        ...
        TEXT
      Benchmark.ips do |x|
        x.report("expand(long)") { r1.self_contained? content }
        x.report("expand(short)") { r1.self_contained? "我们柬埔寨正在招聘菠菜 WX" }
  ...
  describe RuleEngine::Parser do
    it "parse!" do
      ...
      r1.self_contained?("柬埔寨 菠菜 招聘").should eq(true)
      r1.self_contained?("柬埔寨 菠菜").should eq(false)
      ...
      content =
        ...
        TEXT
      Benchmark.ips do |x|
        x.report("parsing(long)") { Parser.parse!(rule_s).self_contained? content }
        x.report("parsing(short)") { Parser.parse!(rule_s).self_contained? "我们柬埔寨正在招聘菠菜 WX" }
      end
    end
  end
end
